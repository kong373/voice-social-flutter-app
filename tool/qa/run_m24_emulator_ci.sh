...
exercise_qa_console_system_input() {
  ...
  if (( passed == 1 )); then
    ...
    if ! grep -q '1 / 60' "$filtered_ui"; then
  ...
  if (( passed == 1 )); then
    record_case \
      ...
      "AC-004 filtered to 1/60, opened, returned with KEYCODE_BACK, scrolled, and reset" \
      "PASS" "" "" \
...
run_page_coverage_shards() {
  ...
  if (( failures == 0 && count_one == 60 && count_one_three == 60 )); then
    ...
    if [[ -z "$first_defect" ]]; then
      record_defect \
        "P1" "page-coverage" \
        "Page shard screenshot cardinality is not exactly 60 at both scales" \
...
generate_page_coverage() {
  ...
  if [[ "$QA_SCOPE_VALUE" == "full" && "$INTEGRATION_SUITE_STARTED" == "1" && \
        "$interaction_count" != "60" ]]; then
    record_defect \
      "P1" "page-coverage" \
      "The Android catalog pass did not emit all 60 per-page interaction markers" \
...
record_existing_p1_inventory() {
  ...
  record_existing_p1_case "M24-EMU-007" "DS/RM/SC" \
    "Visible retired-feature deny-list remains empty across all 60 catalog pages" \
  ...
  record_case "M24-EMU-017" "SC-004" "product-scope" \
    "CP removed by Q23-01" "REMOVED_BY_PRODUCT" "REMOVED_BY_PRODUCT" \
    "" "" "" "" "" "Positive CP workflow retired; see product-exclusions-20260909.md"
