...
validate_log() {
  ...
  if [[ "$PAYMENT_OPT_IN" == 'true' ]]; then
    if [[ "$PAYMENT_CANCEL_ONLY" == 'true' ]]; then
      ...
      [[ "$(grep -Ec '^M5_VENDOR_EVENT::alipay::[^:]+::sdk_callback$' "$log" || true)" -gt 0 ]] || return 1
    fi
  fi
...
validate_db_evidence() {
  ...
  [[ -s "$file" ]] || return 1
  python3 - "$file" "$avd" "$expected_nonce" "$EXPECTED_RUN_ID" "$EXPECTED_FIXTURE_ID" "$EXPECTED_BACKEND_SHA" "$EXPECTED_FLUTTER_SHA" "$apk_sha" "$EXPECTED_BACKEND_DIGEST" "$PAYMENT_OPT_IN" "$PAYMENT_CANCEL_ONLY" "$result_value" <<'PY'
import json
import sys

path, expected_avd, expected_nonce, expected_run_id, expected_fixture, expected_backend_sha, expected_flutter_sha, expected_apk_sha, expected_digest, payment_opt_in, payment_cancel_only, result_value = sys.argv[1:]
...
if (not isinstance(calls, dict) or set(calls) != {"tencentIm", "alipay"} or
    any(type(value) is not int or value < 0 for value in calls.values()) or
    (result_value == "NO_PAY" and calls["tencentIm"] < 1) or
    (payment_opt_in == "true" and calls["tencentIm"] < 1) or
    (payment_opt_in == "true" and calls["alipay"] < 1) or
    (payment_cancel_only == "true" and counters["alipay_orders"] < 1) or
    (payment_cancel_only == "true" and counters["payment_provider_events"] != 0) or
    (payment_cancel_only == "true" and counters["wallet_transactions"] != 0) or
    (payment_cancel_only == "true" and counters["ledger_journals"] != 0) or
    (payment_cancel_only == "true" and counters["ledger_entries"] != 0) or
    (payment_cancel_only == "true" and payload["vendorOutbox"]["alipay"]["attempts"] < 1) or
...
for key in ("tencentIm", "alipay"):
    ...
    if (key == "tencentIm" and calls["tencentIm"] > 0) or (payment_opt_in == "true" and payment_cancel_only != "true"):
...
if [[ "$PAYMENT_OPT_IN" == 'true' && "$PAYMENT_SCENARIO" != 'cancel' ]]; then
...
python3 - "$AGGREGATE_JSON" "$conclusion" "$EXPECTED_FLUTTER_SHA" "$EXPECTED_BACKEND_SHA" "$EXPECTED_BACKEND_DIGEST" "$ANDROID_HOST_SHA" "$EXPECTED_RUN_ID" "$EXPECTED_FIXTURE_ID" "$AVD_A_APK_SHA" "$AVD_B_APK_SHA" "$TOTAL_TENCENT_CALLS" "$TOTAL_ALIPAY_CALLS" "$([[ "$PAYMENT_OPT_IN" == true ]] && printf true || printf false)" "$reasons_text" <<'PY'
import json
import sys
path, conclusion, flutter_sha, backend_sha, backend_digest, android_host_sha, run_id, fixture_id, apk_a, apk_b, tencent_calls, alipay_calls, payment_opt_in, raw_reasons = sys.argv[1:]
...
payload = {
    ...
    "paymentMode": "cancel_only" if payment_opt_in == "true" else "none",
    "secrets": False,
    "reasons": reasons,
}
...

{
  ...
  printf 'apk_a_sha256=%s\napk_b_sha256=%s\npayment_opt_in=%s\npayment_mode=%s\n' "${AVD_A_APK_SHA:-unknown}" "${AVD_B_APK_SHA:-unknown}" "$PAYMENT_OPT_IN" "$([[ "$PAYMENT_CANCEL_ONLY" == true ]] && printf cancel_only || printf none)"
