...
class MockCommunityRepository
  ...
  MockCommunityRepository({GuildRole viewerRole = GuildRole.owner})
    : _guilds = <String, GuildSummary>{
        'guild-1': GuildSummary(
          id: 'guild-1',
          code: 'G10086',
          name: '晚风陪伴社',
          ...
          memberCount: 128,
          ownerUserId: viewerRole == GuildRole.owner ? 10001 : 20001,
          ownerName: viewerRole == GuildRole.owner ? '我' : '晚星',
          role: viewerRole,
          joined: true,
          applicationPending: false,
          hasNewApplications: viewerRole == GuildRole.owner,
      ...
      _members = <GuildMember>[
        GuildMember(
          ...
          userId: 20001,
          nickname: '晚星',
          role: viewerRole == GuildRole.owner
              ? GuildRole.member
              : GuildRole.owner,
          isSigned: true,
          roomId: '880217',
  ...
  Future<void> setGuildMemberMuted({
    ...
    await _delay();
    _requireOwner(await fetchGuild(guildId));
  ...
  static void _requireOwner(GuildSummary guild) {
    _requireActiveGuild(guild);
    if (!guild.joined || !guild.role.canManage) {
      throw const ApiException(
        kind: ApiFailureKind.forbidden,
        message: '仅当前会长可以管理公会',
      );
