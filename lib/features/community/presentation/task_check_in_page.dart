...
class _TaskCheckInPageState extends State<TaskCheckInPage> {
  ...
  Future<void> _sign() async {
    ...
    try {
      final TaskCenterSnapshot value =
          await _repository.completeDailyCheckIn();
      if (mounted) {
                                    ...
                                    trailing: switch (task.state) {
                                      TaskState.claimable => FilledButton.tonal(
                                          onPressed: _busyId == null
                                              ...
                                              : null,
                                          child: Text(
                                            _busyId == task.id ? '领取中…' : '领取',
                                          ),
                                        ),
                                      TaskState.claimed => const _SmallTag(label: '已领取'),
                                      TaskState.expired => const _SmallTag(label: '已过期'),
                                      TaskState.inProgress => const _SmallTag(label: '进行中'),
                                    },
                                  ),
                                ),
                              ),
