...
class _InviteAttributionPageState extends State<InviteAttributionPage> {
  ...
  Future<void> _load() async {
    try {
      final InviteAttribution value = await AppDependencyScope.of(context)
          .communityRepository
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: <Widget>[
                              ...
                              _KeyValue(label: '已邀请用户', value: '${data.invitedUsers} 人'),
  ...
  Future<void> _request() async {
    ...
    if (userId == null || _busy) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('请输入有效用户 ID')),
      );
      return;
    }
    setState(() => _busy = true);
    try {
      final CpEligibility eligibility = await _repository.checkCpEligibility(userId);
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('CP 邀请已发送，等待对方确认')),
        );
      }
    } catch (error) {
      if (mounted) {
