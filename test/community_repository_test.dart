...
void main() {
  test('guild operations target exact applications and member records', () async {
    ...
    final List<GuildApplication> applications =
        await repository.fetchGuildApplications(guildId);
    ...
    final List<GuildApplication> remaining =
        await repository.fetchGuildApplications(guildId);
    expect(remaining.any((GuildApplication item) => item.id == secondId), isFalse);
    expect(remaining.any((GuildApplication item) => item.id == applications.first.id), isTrue);

    final GuildMember target = (await repository.fetchGuildMembers(guildId))
  ...
  test('CP, guardian, task, and activity state remains authoritative', () async {
    final MockCommunityRepository repository = MockCommunityRepository();
    final CpInvitation invitation =
        (await repository.fetchPendingCpInvitations()).first;
    ...
    expect(
      (await repository.fetchCpRelations())
          .any((CpRelation relation) => relation.userId == invitation.userId),
      isTrue,
    );

    final GuardianFanSnapshot before = await repository.fetchGuardianFan(20001);
    ...
    final TaskItem claimable = tasks.tasks
        .firstWhere((TaskItem item) => item.state == TaskState.claimable);
    final TaskCenterSnapshot claimed = await repository.claimTask(claimable.id);
    expect(
      claimed.tasks
  ...
  test('duplicate or stale community actions fail instead of faking success', () async {
    final MockCommunityRepository repository = MockCommunityRepository();
