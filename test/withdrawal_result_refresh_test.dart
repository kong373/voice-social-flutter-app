...
void main() {
  group('W4 withdrawal recovery', () {
    test(
      'unknown then same-identity 403 preserves exact intent until explicit recovery',
      () async {
        final h = _Harness();
        addTearDown(h.close);
        var attempts = 0;
        h.override = (request) {
          ...
          attempts++;
          if (attempts == 1)
            return MediaFakeResponse.json(null, status: 503, code: 50301);
          if (attempts == 2)
            return MediaFakeResponse.json(null, status: 403, code: 40322);
        ...
        expect(
          ...
          hasLength(2),
          reason:
              'Neither refusal nor an altered intent may start a replacement POST',
        );
        ...
        expect(
          h.http.requests.where((r) => r.uri.path.endsWith('/fee-rate')),
          isEmpty,
        );
      },
    );
    ...
    test(
      'first definitive refusal clears the new intent and permits a separately confirmed request',
      () async {
        final h = _Harness();
        addTearDown(h.close);
        var attempts = 0;
    ...
    test(
      'late A refusal cannot clear B pending command and B cannot adopt A key',
      () async {
        final h = _Harness();
        addTearDown(h.close);
        ...
        b.complete(
          MediaFakeResponse.json(_receipt(amount: 200, id: 'withdrawal-b')),
        );
    ...
    test(
      'ABA late original completion cannot clear a newer explicit recovery flight',
      () async {
        final h = _Harness();
        addTearDown(h.close);
        ...
        await expectLater(
          h.submit(account: 'different'),
          throwsA(isA<ApiException>()),
        );
        expect(h.posts, hasLength(2));
        expect(
          h.posts.first.headers.value('X-Request-Id'),
          h.posts.last.headers.value('X-Request-Id'),
        );
  ...
  group('W4 identity-bound withdrawal reads', () {
    for (final kind in ['quote', 'list', 'filtered', 'record']) {
      for (final aba in [false, true]) {
        test(
          '$kind rejects delayed-open identity switch aba=$aba without sending',
          () async {
            final h = _Harness();
            addTearDown(h.close);
            ...
            expect(
              h.http.requests.single.headers.value(
        ...
        test(
          '$kind stops a 401 recovery after identity switch aba=$aba',
          () async {
      ...
      test(
        '$kind permits same-identity token refresh with unchanged query',
        () async {
          final identity = TestMediaIdentity();
          var refreshes = 0;
          final h = _Harness(
          ...
          expect(
            h.http.requests.first.headers.value(
              HttpHeaders.authorizationHeader,
            ),
            'Bearer contract-A-old',
          );
          expect(
            h.http.requests.last.headers.value(HttpHeaders.authorizationHeader),
      ...
      for (final failure in [false, true]) {
        test(
          '$kind rejects late ${failure ? 'error' : 'success'} for an obsolete identity',
          () async {
            final h = _Harness();
            addTearDown(h.close);
            final gate = h.gate();
            ...
            if (failure)
              gate.completeError(
                const SocketException('obsolete transport failure'),
              );
            else
              gate.complete(MediaFakeResponse.json(h.data(request)));
            await result;
            expect(h.http.requests, hasLength(1));
            expect(h.posts, isEmpty);
          },
    ...
    for (final kind in ['filtered', 'record']) {
      test(
        '$kind cannot rebind between pages when identity changes at the first body boundary',
        () async {
          final h = _Harness();
          addTearDown(h.close);
          h.history = List.generate(
            101,
            (index) => _record(id: 'withdrawal-${index + 1}'),
          );
          h.override = (request) {
            if (request.uri.queryParameters['pageNum'] == '1') {
              ...
              return MediaFakeResponse(
                200,
                _changeAfterBody(bytes, h.identity),
              );
            }
            return null;
          };
          await expectLater(
            _read(h, kind, recordId: 'withdrawal-101'),
            throwsA(_staleIdentity),
          );
          expect(
            h.http.requests,
            hasLength(1),
            reason: 'A later page must never capture the new account',
          );
          expect(h.posts, isEmpty);
        },
      );
...
void _widgetTests() {
  for (final oldError in [false, true]) {
    testWidgets(
      'W4 post-submit readback wins over earlier refresh error=$oldError',
      (tester) async {
        ...
        await _withPage(tester, h, (dependencies) async {
          ...
          await _until(
            ...
            'Application POST must reach the transport barrier',
          );
          final prior = refresh();
          await _until(
            tester,
            () => oldStarted.isCompleted,
            'Earlier refresh must reach the held GET',
          );
          h.cash = 200;
          ...
          await _until(
            tester,
            () =>
                recordReads == 3 && find.text('提现申请已提交').evaluate().isNotEmpty,
            'Confirmed application must finish its authoritative readback',
          );
          expect(find.text('可提现 ¥200.00'), findsOneWidget);
          expect(find.textContaining('****NEW'), findsOneWidget);
          if (oldError)
            old.completeError(const SocketException('obsolete refresh'));
          else
            old.complete(MediaFakeResponse.json(_recordPage([], size: 50)));
    ...
    testWidgets(
      'W4 old refresh error=$oldError cannot clear a newer loading state',
      (tester) async {
        final h = _Harness();
        ...
        await _withPage(tester, h, (dependencies) async {
          ...
          await _until(
            tester,
            () => reads == 3,
            'Second refresh request barrier',
          );
          if (oldError)
            old.completeError(const SocketException('obsolete refresh'));
          else
            old.complete(
              MediaFakeResponse.json(
                _recordPage([_record(mask: '****OLD')], size: 50),
              ),
            );
          await _finish(tester, prior);
          ...
          latest.complete(
            MediaFakeResponse.json(
              _recordPage([
                _record(status: 'SETTLED', mask: '****NEW'),
              ], size: 50),
            ),
          );
  ...
  for (final stage in ['records', 'accounts']) {
    testWidgets(
      'W4 replacement repository invalidates old $stage response and intermediate hints',
      (tester) async {
        final h = _Harness();
        final replacement = _Harness()..cash = 450;
        replacement.history = [
          _record(status: 'SETTLED', mask: '****REPLACEMENT'),
        ];
        final old = h.gate();
        ...
        await _withPage(tester, h, (dependencies) async {
          ...
          await _until(
            tester,
            () => held.isCompleted,
            'Old scope must reach the selected stage',
          );
          try {
            await _mount(
              tester,
              ...
              replacement,
              stage:
                  'Replacement repository must issue and complete a new scope read',
            );
            ...
            expect(
              ...
              oldAccountReads,
              reason:
                  'An obsolete multi-stage read must not start another account GET',
            );
            expect(h.posts, isEmpty);
  ...
  testWidgets(
    'W4 disposed withdrawal page does not continue the next read stage',
    (tester) async {
      final h = _Harness();
      final old = h.gate();
      final held = Completer<void>();
      await _withPage(tester, h, (dependencies) async {
        ...
        await _until(
          tester,
          () => held.isCompleted,
          'Held history request before disposal',
        );
        final accounts = h.accountReads;
        await tester.pumpWidget(const SizedBox.shrink());
        old.complete(
          MediaFakeResponse.json(_recordPage([_record()], size: 50)),
        );
        await _finish(tester, prior);
  ...
  testWidgets(
    'W4 same-page account ABA ignores a late failure without hiding the current result',
    (tester) async {
      final h = _Harness();
      final old = h.gate();
      final held = Completer<void>();
      ...
      await _withPage(tester, h, (dependencies) async {
        ...
        await _until(
          tester,
          () => held.isCompleted,
          'Held old-account history request',
        );
        h.cash = 470;
        h.identity.change(2);
        await _until(
          tester,
          () => find.text('可提现 ¥470.00').evaluate().isNotEmpty,
          'B view ready',
        );
        h.identity.change(1);
        await _until(
...
Stream<List<int>> _changeAfterBody(
  List<int> bytes,
...
WithdrawalQuote _quote({double amount = 100, int version = 7}) =>
    WithdrawalQuote(
      quotedAmount: amount,
      feeAmount: 0,
...
class _Harness {
  _Harness({TestMediaIdentity? identity, Future<bool> Function()? refresh})
    ...
    repository = BackendCommerceRepository(
      ...
      currentUserId: () =>
          this.identity.user == 0 ? null : '${this.identity.user}',
  ...
  List<MediaFakeRequest> get posts =>
      http.requests.where((r) => r.method == 'POST').toList();
  ...
  Future<WithdrawalRecord> submit({
    double amount = 100,
    WithdrawalQuote? quote,
    String account = 'account-1',
  }) => repository.applyWithdrawal(
    amount: amount,
    confirmedQuote: quote ?? _quote(amount: amount),
  ...
  Object? data(MediaFakeRequest request) {
    ...
    if (path.endsWith('/wallet/overview'))
      return {
        'incomeRole': 'ANCHOR',
        'incomeEligible': true,
        'canWithdraw': true,
    ...
    if (path.endsWith('/accounts'))
      return {
        ...
        'total': 1,
        'selectedPayoutAccountId': 'account-1',
        'defaultPayoutAccountId': 'account-1',
        'selectionRequired': false,
        'providerInvocation': false,
        'canBind': true,
        'bindingBlockReason': 'NONE',
      };
    if (path.endsWith('/fee-rate'))
      return {
        'amountMinor': int.parse(request.uri.queryParameters['amountMinor']!),
        'feeMinor': 0,
        'netAmountMinor': int.parse(
          request.uri.queryParameters['amountMinor']!,
        ),
  ...
  void releaseGates() {
    for (final value in _gates) {
      if (!value.isCompleted)
        value.complete(MediaFakeResponse.json(null, status: 503, code: 50301));
    }
  }
...
Future<void> _until(
  ...
  fail(
    'Did not reach observed W4 barrier: $stage. Initial fixture failure is not business RED.',
  );
}
...
Future<void> _mount(
  ...
  _Harness h, {
  String stage =
      'Initial fixture: wallet, records and accounts must load before controls are used',
}) async {
  await tester.pumpWidget(
    AppDependencyScope(
      ...
      child: MaterialApp(
        ...
        home: WithdrawalPage(
          key: const ValueKey('withdrawal-scope'),
...
Future<void> _confirmApplication(WidgetTester tester, _Harness h) async {
  ...
  await _until(
    tester,
    () => find.textContaining('服务端报价：').evaluate().isNotEmpty,
    'Valid quote must render before confirmation',
  );
  ...
  await _until(
    tester,
    () => find.text('确认提现').evaluate().isNotEmpty,
    'Explicit confirmation dialog',
  );
  expect(
    h.posts,
    isEmpty,
    reason: 'Opening the dialog is not authorization to send',
  );
