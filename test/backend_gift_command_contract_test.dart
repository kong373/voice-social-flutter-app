...
void main() {
  ...
  final invalidAmounts = <String, Map<String, Object?>>{
    'negative total': {'giftCoinCost': -1},
    'conflicting total aliases': {'giftCoinCost': 100, 'totalCostMinor': 101},
    'unit quantity mismatch': {'giftCoinCost': 99, 'unitCostMinor': 10},
    'wrong consumption currency': {'giftCoinCost': 100, 'currency': 'USD'},
    'currency without amount': {'currency': 'CASH_CNY'},
    'nested recipient mismatch': {
      'giftCoinCost': 100,
      'cost': {'totalCostMinor': 100, 'receiverUserId': 10003},
    },
  };
  ...
  test('gift receipt amount preserves absent optional amounts', () async {
    ...
    addTearDown(h.close);
    final command = h.command();
    final absent = await h.repo.queryGiftCommand(command);
    command.validateReceipt(absent, queried: true);
    // A correct optional consumption currency does not invent a required price.
    h.patch = {'currency': 'GIFT_COIN'};
    final currencyOnly = await h.repo.queryGiftCommand(command);
    command.validateReceipt(currencyOnly, queried: true);
    expect(currencyOnly.transferId, absent.transferId);
    expect(h.gets, 2);
    expect(h.bodies, isEmpty);
  });
