...
void main() {
  test('platform channels and youth-mode recharge boundary are exact', () async {
  ...
  test('payment return waits for server order confirmation', () async {
    final MockCommerceCatalogRepository repository =
        MockCommerceCatalogRepository();
    final MembershipSnapshot before =
        await repository.fetchMembershipSnapshot();
    final RechargeProduct product =
        (await repository.fetchRechargeProducts(
      platform: ClientStorePlatform.android,
    ))
    ...
    final MembershipSnapshot after =
        await repository.fetchMembershipSnapshot();
    expect(
  ...
  test('ordinary gifts and decoration state exclude retired capabilities', () async {
    final MockCommerceCatalogRepository repository =
        MockCommerceCatalogRepository();
    final List<GiftCatalogItem> gifts = await repository.fetchGiftCatalog();
    final String names = gifts.map((GiftCatalogItem item) => item.name).join(' ');
