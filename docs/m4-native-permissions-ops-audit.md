...
| 内容/账号审核 | `AC-006` 实名提交与状态、`AC-009` 处罚申诉、`US-008` 举报、`SC-002` 入会申请审核、`CM-007/CM-008` 退款申请状态 | 实名人工审核队列、证件材料查看/脱敏、审核员决策、举报/申诉工作台、审核审计与批量操作 | C 端只展示服务端状态；canonical 审核状态为 `VERIFIED`，实名业务状态仍按服务端 `PENDING/APPROVED/REJECTED` 显示 |
